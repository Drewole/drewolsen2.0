...
function addParticleEffect(canvasSelector, userOptions) {
	...
	function randomNorm(mean, stdev) {
		return (
			Math.abs(Math.round(Math.random() * 2 - 1 + (Math.random() * 2 - 1) + (Math.random() * 2 - 1)) * stdev) +
			mean
		);
	}
	...
	function Particle() {
		...
		this.s = parseInt(90 * Math.random() + 90, 10);
		this.l = parseInt(90 * Math.random() + 90, 10);
